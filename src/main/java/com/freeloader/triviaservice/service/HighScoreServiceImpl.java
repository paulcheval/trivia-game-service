...
public class HighScoreServiceImpl implements HighScoreService {
	...
	public HighScoreResponse getAllHighScores() {
		return new HighScoreResponse(
				objectConversion.convertHighScoreToHighScoreDetail(
						retrieveHighScoresFromRepository()));
	}

	@Override
	public void addHighScore(HighScoreRequest highScoreRequest) throws Exception {
		if (isNewHighScoreTooLowToConsider(highScoreRequest)) { throw new HighScoreNotConsideredForAdding(); }
	...
	private int determineMinScoreInDB() {
		return retrieveHighScoresFromRepository().stream()
				.sorted(Comparator.comparing(HighScore::getScore))
	...
	private void cleanUpLowScores(List<HighScoreDetail> lowScores) {
		...
		lowScores.forEach(score -> {
		    repository.findByNameAndScore(score.name(), score.score())
		        .forEach(lowScore -> {
		...
		idsToDelete.forEach(id -> {
			log.info("Deleting entries with low score with name: " + repository.findById(id).get().getName() + " and score: " + repository.findById(id).get().getScore());
